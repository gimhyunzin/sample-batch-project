...
@Component
@StepScope
class ClaimCompleteAlarmKkoProcessor : ItemProcessor<MutableList<AdminAlarm>, MutableList<Msg>> {

    private val logger = LoggerFactory.getLogger(ClaimCompleteAlarmKkoProcessor::class.java)
    ...
    private fun createReceiverList(data: AdminAlarm): MutableList<Receiver> {
        ...
        logger.info("====== [CLAIM_COMPLETE_ALARM] ====== MD  ($md) ====== : ${String.format("%-11s", AlarmHelper.maskMobile(mdMobile))} / $nowDateTime")
        logger.info("====== [CLAIM_COMPLETE_ALARM] ====== AMD ($amd) ====== : ${String.format("%-11s", AlarmHelper.maskMobile(amdMobile))} / $nowDateTime")
        logger.info("====== [CLAIM_COMPLETE_ALARM] ====== CS  ($cs) ====== : ${String.format("%-11s", AlarmHelper.maskMobile(csMobile))} / $reqDateTime")
        logger.info("====== [CLAIM_COMPLETE_ALARM] ====== AFF ($aff) ====== : ${String.format("%-11s", AlarmHelper.maskMobile(affMobile))} / $reqDateTime")
        logger.info("====== [CLAIM_COMPLETE_ALARM] ====== INV ($inventory) ====== : ${String.format("%-11s", AlarmHelper.maskMobile(inventoryMobile))} / $reqDateTime")
