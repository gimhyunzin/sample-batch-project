...
@Component
@StepScope
class ClaimRequestAlarmKkoProcessor : ItemProcessor<MutableList<AdminAlarm>, MutableList<Msg>> {

    private val logger = LoggerFactory.getLogger(ClaimRequestAlarmKkoProcessor::class.java)
    ...
    private fun createReceiverList(data: AdminAlarm): MutableList<Receiver> {
        ...
        logger.info("====== [CLAIM_REQUEST_ALARM] =================== [DEAL_ID] ${dealInfo.dealId} =================== ")
        logger.info("====== [CLAIM_REQUEST_ALARM] ====== MD  ($md) ====== : ${String.format("%-11s", AlarmHelper.maskMobile(mdMobile))} / $nowDateTime")
        logger.info("====== [CLAIM_REQUEST_ALARM] ====== AMD ($amd) ====== : ${String.format("%-11s", AlarmHelper.maskMobile(amdMobile))} / $nowDateTime")
        logger.info("====== [CLAIM_REQUEST_ALARM] ====== CS  ($cs) ====== : ${String.format("%-11s", AlarmHelper.maskMobile(csMobile))} / $reqDateTime")
        logger.info("====== [CLAIM_REQUEST_ALARM] ====== AFF ($aff) ====== : ${String.format("%-11s", AlarmHelper.maskMobile(affMobile))} / $reqDateTime")
        logger.info("====== [CLAIM_REQUEST_ALARM] ====== INV ($inventory) ====== : ${String.format("%-11s", AlarmHelper.maskMobile(inventoryMobile))} / $reqDateTime")
    ...
    private fun convertMsgTemplate(data: AdminAlarm): AdminAlarmMsgTemplate {
        ...
        messageSubject = messageSubject.replace(oldValue = "{0}", newValue = "구매취소")
