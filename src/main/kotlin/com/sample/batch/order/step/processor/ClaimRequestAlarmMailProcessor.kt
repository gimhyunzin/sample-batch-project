...
@Component
@StepScope
class ClaimRequestAlarmMailProcessor : ItemProcessor<MutableList<AdminAlarm>, MutableList<Mail>> {

    private val logger = LoggerFactory.getLogger(ClaimRequestAlarmMailProcessor::class.java)
    ...
    private fun createMailList(data : AdminAlarm): MutableList<Mail> {
        ...
        logger.info("====== [CLAIM_REQUEST_ALARM] ====== MD  ($md) ====== : ${AlarmHelper.maskName(mdName)} / ${AlarmHelper.maskEmail(mdEmail)}")
        logger.info("====== [CLAIM_REQUEST_ALARM] ====== AMD ($amd) ====== : ${AlarmHelper.maskName(amdName)} / ${AlarmHelper.maskEmail(amdEmail)}")
        logger.info("====== [CLAIM_REQUEST_ALARM] ====== CS  ($cs) ====== : ${AlarmHelper.maskName(csName)} / ${AlarmHelper.maskEmail(csEmail)}")
        logger.info("====== [CLAIM_REQUEST_ALARM] ====== AFF ($aff) ====== : ${AlarmHelper.maskName(affName)} / ${AlarmHelper.maskEmail(affEmail)}")
        logger.info("====== [CLAIM_REQUEST_ALARM] ====== INV ($inventory) ====== : ${AlarmHelper.maskName(inventoryName)} / ${AlarmHelper.maskEmail(inventoryEmail)}")
    ...
    private fun createMailInfo(data: AdminAlarm): Mail {
        ...
        return Mail(
                ...
                // 취소일시
                tag1 = AlarmHelper.convertMailDate(data.requestDate),
                // 딜 명
