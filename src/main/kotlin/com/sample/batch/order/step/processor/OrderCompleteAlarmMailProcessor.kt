...
@Component
@StepScope
class OrderCompleteAlarmMailProcessor : ItemProcessor<MutableList<AdminAlarm>, MutableList<Mail>> {

    private val logger = LoggerFactory.getLogger(OrderCompleteAlarmMailProcessor::class.java)
    ...
    private fun createMailList(data : AdminAlarm): MutableList<Mail> {
        ...
        logger.info("====== [ORDER_COMPLETE_ALARM] ====== MD  ($md) ====== : ${AlarmHelper.maskName(mdName)} / ${AlarmHelper.maskEmail(mdEmail)}")
        logger.info("====== [ORDER_COMPLETE_ALARM] ====== AMD ($amd) ====== : ${AlarmHelper.maskName(amdName)} / ${AlarmHelper.maskEmail(amdEmail)}")
        logger.info("====== [ORDER_COMPLETE_ALARM] ====== CS  ($cs) ====== : ${AlarmHelper.maskName(csName)} / ${AlarmHelper.maskEmail(csEmail)}")
        logger.info("====== [ORDER_COMPLETE_ALARM] ====== AFF ($aff) ====== : ${AlarmHelper.maskName(affName)} / ${AlarmHelper.maskEmail(affEmail)}")
        logger.info("====== [ORDER_COMPLETE_ALARM] ====== INV ($inventory) ====== : ${AlarmHelper.maskName(inventoryName)} / ${AlarmHelper.maskEmail(inventoryEmail)}")
    ...
    private fun createMailInfo(data: AdminAlarm): Mail {
        ...
        return Mail(
                autoType = BatchMailType.ORDER_ALARM.type,

                // 구매일시
                tag1 = AlarmHelper.convertMailDate(data.orderDate),
                // 딜 명
