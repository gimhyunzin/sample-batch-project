...
import com.sample.batch.common.property.ShareKey.CLAIM_COMPLETE_DATA_LIST
...
@Component
@StepScope
class ClaimCompleteAlarmKkoReader : ItemReader<MutableList<AdminAlarm>> {

    private val logger = LoggerFactory.getLogger(ClaimCompleteAlarmKkoReader::class.java)
    ...
    override fun read(): MutableList<AdminAlarm>? {
        ...
        return when (isRead) {
            ...
            false -> {
                ...
                val claimCompleteList = mapper.selectClaimCompleteList(params)
                dataList = claimCompleteList
    ...
    fun saveShareStepExecution(stepExecution: StepExecution) {
        shareData.putData(CLAIM_COMPLETE_DATA_LIST, dataList)
        ...
        logger.info("========= [CLAIM_COMPLETE_DATA_LIST] >>>> size : [${dataList.size}] =========")
    }
}