...
import com.sample.batch.common.property.ShareKey.CLAIM_COMPLETE_LAST_KEY
...
@Component
@StepScope
class ClaimCompleteAlarmMailReader : ItemReader<MutableList<AdminAlarm>> {
    ...
    fun loadShareStepExecution(stepExecution: StepExecution) {
        this.dataList = shareData.getData(CLAIM_COMPLETE_DATA_LIST) as MutableList<AdminAlarm>? ?: mutableListOf()
    }

    @AfterStep
    fun saveShareStepExecution(stepExecution: StepExecution) {
        val lastClaimId = if (dataList.isEmpty()) 0 else dataList[0].claimId
        shareData.putData(CLAIM_COMPLETE_LAST_KEY, lastClaimId)
    }
}