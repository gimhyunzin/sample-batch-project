...
@Component
@StepScope
class ClaimRequestAlarmKkoReader : ItemReader<MutableList<AdminAlarm>> {

    private val logger = LoggerFactory.getLogger(ClaimRequestAlarmKkoReader::class.java)
    ...
    override fun read(): MutableList<AdminAlarm>? {
        ...
        return when (isRead) {
            ...
            false -> {
                ...
                val claimRequestList = mapper.selectClaimRequestList(params)
                dataList = claimRequestList
    ...
    fun saveShareStepExecution(stepExecution: StepExecution) {
        shareData.putData(CLAIM_REQUEST_DATA_LIST, dataList)
        ...
        logger.info("========= [CLAIM_REQUEST_DATA_LIST] >>>> size : [${dataList.size}] =========")
    }
}